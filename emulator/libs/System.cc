...
void System::registerModules() {
	size_t mem_size = acalsim::top->getParameter<int>("System", "memory_size");

	// create modules
	cpu  = new CPUEmulator("CPU Emulator");
...
bool System::streq(char* s, const char* q) {
	if (strcmp(s, q) == 0) return true;

	return false;
}

uint32_t System::signextend(uint32_t in, int bits) {
	if (in & (1 << (bits - 1))) return ((-1) << bits) | in;
	return in;
}

void System::print_syntax_error(int line, const char* msg) { ERROR << "Line " << line << ": Syntax error! " << msg; }

void System::append_source(const char* op, const char* a1, const char* a2, const char* a3, source* src, instr* i) {
...
int System::parse_reg(char* tok, int line, bool strict) {
	if (tok[0] == 'x') {
		int ri = atoi(tok + 1);
		if (ri < 0 || ri > 32) {
...
uint32_t System::parse_imm(char* tok, int bits, int line, bool strict) {
...
void System::parse_mem(char* tok, int* reg, uint32_t* imm, int bits, int line) {
...
int System::parse_data_element(int line, int size, uint8_t* mem, int offset) {
...
int System::parse_assembler_directive(int line, char* ftok, uint8_t* mem, int memoff) {
...
int System::parse_pseudoinstructions(int line, char* ftok, instr* imem, int ioff, label_loc* labels, char* o1, char* o2,
...
instr_type System::parse_instr(char* tok) {
	// instruction added
...
int System::parse_instr(int line, char* ftok, instr* imem, int memoff, label_loc* labels, source* src) {
...
void System::parse(const std::string& file_path, uint8_t* mem, instr* imem, int& memoff, label_loc* labels,
...
uint32_t System::label_addr(char* label, label_loc* labels, int label_count, int orig_line) {
...
void System::normalize_labels(instr* imem, label_loc* labels, int label_count, source* src) {
